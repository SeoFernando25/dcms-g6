...
import { Component, OnInit, ChangeDetectionStrategy, HostListener, ElementRef, Injectable } from '@angular/core';
...
export class TestimonialsComponent implements OnInit {
  ...
  onSubmit(): void {
      ...
      .insert([
        {
          ...
          value_score: this.reviewForm.value.reviewScore ? this.reviewForm.value.reviewScore : 0,
        }
      ]).then((response) => {
    ...
    this.snackBar.open('Your review has been submitted!', 'Dismiss').afterDismissed().subscribe(() => {
      window.location.reload();
    });
  }
  ...
  onReviewClick(reviewId: number): void {
    ...
    this.supabase._supabase.from('review').delete().eq('id', reviewId).then((response) => {
      console.log(response);
    });
    this.snackBar.open('Your review has been deleted!', 'Dismiss').afterDismissed().subscribe(() => {
      window.location.reload();
    });
  }
}
...
export class ReviewDataSource extends DataSource<Review | undefined> {
  ...
  private readonly _dataStream = new BehaviorSubject<(Review | undefined)[]>(this._cachedData);
  ...
  connect(collectionViewer: CollectionViewer): Observable<(Review | undefined)[]> {
    this._fetchPage(0);
